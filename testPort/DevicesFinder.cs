        ...
        public static PnPEntityInfo[] WhoUsbDevice(UInt16 VendorID, UInt16 ProductID, Guid ClassGuid)
        {
            ...
            if (USBControllerDeviceCollection != null)
            {
                foreach (ManagementObject USBControllerDevice in USBControllerDeviceCollection)
                {
                   ...
                   if(match.Success && Dependent.Contains("&3"))
                    {
                        //UInt16 theVendorID = Convert.ToUInt16(Dependent.Substring(4, 4), 16);   // 供应商标识
                        //if (VendorID != UInt16.MinValue && VendorID != theVendorID) continue;
                       // UInt16 theProductID = Convert.ToUInt16(Dependent.Substring(13, 4), 16); // 产品编号
                       // if (ProductID != UInt16.MinValue && ProductID != theProductID) continue;
                        ManagementObjectCollection PnPEntityCollection = new ManagementObjectSearcher("SELECT * FROM Win32_USBControllerDevice WHERE Dependent=" + Dependent).Get();
